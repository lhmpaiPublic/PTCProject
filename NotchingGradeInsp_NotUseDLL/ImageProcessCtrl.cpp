...
int CImageProcessCtrl::ResetCamera()
{
	...
	// ¿ÃπÃ¡ˆ Cut Ω∫∑°µÂ ∞¥√º ¡æ∑· »ƒ ¥ŸΩ√ ª˝º∫ ƒ´∏ﬁ∂Û∞° Reset µ… ∞ÊøÏ
	if (m_pImgCutTabThread != NULL) {
		//¿ÃπÃ¡ˆ Cut Ω∫∑°µÂ Kill
	...
	//Grabber º˝¿⁄ ∏∏≈≠ Frame ≈•∏¶ Reset
	for (int i = 0; i < GRABBER_COUNT; i++) {
		m_pQueueFrmCtrl[i]->ResetQueue();	// 22.05.19 Ahn Add 

		//Grabber ∞¥√º∏¶ ª˝º∫ «œ∞Ì  Dalsa ƒ´∏ﬁ∂Û ø¨∞·
...
int CImageProcessCtrl::Destroy()
{
	...
	//¿ÃπÃ¡ˆ Cut Ω∫∑°µÂ ªÏæ∆ ¿÷¥Ÿ∏È Kill »ƒ ∞≥∞‘ ªË¡¶
	if (m_pImgCutTabThread != NULL) {
		m_pImgCutTabThread->Kill();
		delete m_pImgCutTabThread;
...
int CImageProcessCtrl::GrabStart(BOOL bMode)
{
	...
	if (bError == TRUE) {
		CString strMsg;
		strMsg.Format(_T("Ïπ¥Î©îÎùºÍ∞Ä Ïù¥ÎØ∏ Ïó∞Í≤∞Ï§ë ÏûÖÎãàÎã§."));
...
{
	if (m_bInspFlag == TRUE) {
		AfxMessageBox(_T("Í≤ÄÏÇ¨Ï§ë ÏûÖÎãàÎã§."));
		return 1;
	}
