...
CImageProcThread::CImageProcThread(CImageProcessCtrl *pParent)
{
	...
	m_pFrame = NULL;
	m_pFrame = (CMainFrame * )AfxGetApp()->GetMainWnd();
}
				...
				while (1)
				{
					...
					//Tab  있는 포인터 만 값을 세팅 하면 같이 변한다.
						...
						{ // CSV 파일 작성
							...
							strResult.Format(_T("%s,%d,%d,%d,%s,%s,%s,%s,%d,%d,%.2lf,%.2lf,%s,%s,%s,%s,%.2lf,%s,%.2lf,%d,%d,%d,%d,%d,%.2lf,%s,%.2lf,%s,%.2lf,%s,%.2lf,%s,%.2lf,%s,%.2lf,%s\r\n")
								, AprData.m_NowLotData.m_strLotNo
								, pTopInfo->nTabNo + 1
								, pTopInfo->m_nTabId_CntBoard
								, nKeyId
