...
void CNotchingGradeInspView::MelsecTheadRun()
{

	//이벤트 객체 생성

	if ((m_pThread = AfxBeginThread((AFX_THREADPROC)ThreadProc_Melsec,
		(LPVOID)this,
...
void CNotchingGradeInspView::MelsecTheadStop()
{
...
UINT ThreadProc_Melsec(LPVOID Param)
{
	while (TRUE)
	{
		CNotchingGradeInspView* pView = (CNotchingGradeInspView*)Param;

		CSigProc* pSigProc = theApp.m_pSigProc; 

		if (pView == NULL || pSigProc == NULL)
		{
			Sleep(1);
			continue;
		}
		...
		pView->CheckLotEndProcess2();
		pView->CheckTabZeroReset();
		pView->CheckLotStartProcess();
		pView->CheckAlarmReset();
		pView->CheckRecipeChange();
		pView->CheckInkMarkActive();
		pView->CheckConnectZone();
		pView->CheckAlarmNgAck();
		...
		case	CNotchingGradeInspView::en_CameraReset:
			...
			{
				pView->m_nCamErrorResetCnt = 0;
				pView->ChangeStatus(enInspRun);
			}
			else
			{
				if (pView->m_nCamErrorResetCnt > 3)
				{
					pView->ChangeStatus(enInspStop);
					pView->m_nStatus = CNotchingGradeInspView::en_Initialize;
				}
				else
				{ // 3회 까지 Retry
					pView->m_nStatus = CNotchingGradeInspView::en_CameraReset;
					break;
				}
				break;
			}
		...
		case	CNotchingGradeInspView::en_InspStop:
			...
			{
				pView->m_nStatus = CNotchingGradeInspView::en_WaitReady;
			...
			else
			{
				...
				{
					if (pView->m_bDebugLotStartReq == TRUE)
					{
						AprData.LotStartProcess(TRUE, 0x0001);
						pView->m_bDebugLotStartReq = FALSE;
					}
					else if (pView->m_bDebugLotEndReq == TRUE)
					{
...
//						pFrame->ResetAndRefreshAll();
		...
		case	CNotchingGradeInspView::en_WaitReady:
			...
			if (AprData.m_DebugSet.GetDebug(CDebugSet::en_Debug_Melsec) == TRUE)
			{
				pView->m_nStatus = CNotchingGradeInspView::en_Ready;
				if (AprData.LotStartProcess(FALSE) < 0)
				{
					pView->m_nStatus = CNotchingGradeInspView::en_Initialize;
					AfxMessageBox(_T("LotStartProcess Error !! Please Check Recipe Parameter"));
					break;
				}
			}
			...
			if (pSigProc->SigInReady() == TRUE)
			{
				...
				{
					pView->m_pDeleteThread->Kill();
					delete pView->m_pDeleteThread;
					pView->m_pDeleteThread = nullptr;
				}
				*/
			}
		...
		case	CNotchingGradeInspView::en_Ready:
		...
		case	CNotchingGradeInspView::en_PrepareRun:
			...
			if (pView->m_bLotStartInitFlag == TRUE)
			{
				if (AprData.LotStartProcess(FALSE) < 0)
				{
					pView->m_nStatus = CNotchingGradeInspView::en_Initialize;
				}
				pView->CameraGrabStart();
...
//				pFrame->ReflashAll();
			...
			if (AprData.m_DebugSet.GetDebug(CDebugSet::en_Debug_Melsec) == TRUE) {
				...
				int nRecipeNo = 0;
				CNotchingGradeInspDoc* pDoc = (CNotchingGradeInspDoc*)pView->m_pDocument;
			...
			pView->m_bEncoderReset = FALSE;
			pView->m_nStatus = CNotchingGradeInspView::en_Run;
			pView->m_nRedrawCnt = 0;

			pView->InspectionStart();
			break;
		...
		case	CNotchingGradeInspView::en_Run:
			if (pView->IsInspReady() == FALSE)
			{
				// Stop 버튼을 누른경우.
				pView->m_nStatus = CNotchingGradeInspView::en_Initialize;
			...
			if (pView->m_nRedrawCnt >= 50)
			{
				CRect rcRect;
				pView->ReDrawMap(FALSE, rcRect);
				pView->m_nRedrawCnt = 0;
			}
			else
			{
...
//				pFrame->ReflashAll();
		...
		case	CNotchingGradeInspView::en_LotChange:
			pView->m_nStatus = CNotchingGradeInspView::en_Initialize;
			break;
		...
		case	CNotchingGradeInspView::en_ErrorStop:
			break;
