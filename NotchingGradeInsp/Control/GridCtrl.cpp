...
#include "GridCellSpinCtrl.h"
#include "GridCellClick.h"
...
//	m_bSetEditableInit = FALSE ;
	...
	if ( pDC != NULL ) {
		m_xx = pDC->GetDeviceCaps( LOGPIXELSX ) / ( double)1440.0 ;
		m_yy = pDC->GetDeviceCaps( LOGPIXELSY ) / ( double)1440.0 ;
...
LRESULT CGridCtrl::SendCacheHintToParent(const CGridCellRange& range) const
{
    if (!IsWindow(m_hWnd))
        return 0;
...
{  
    ...
    EndEditing();        // destroy any GridInPlaceEdit's
...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
    ...
    CGridCellID next = m_idCurrentCell;
    ...
    {
        CGridCellID idOldTopLeft = GetTopleftNonFixedCell();
        SendMessage(WM_VSCROLL, SB_PAGEDOWN, 0);
		bVertScrollAction = TRUE;
        CGridCellID idNewTopLeft = GetTopleftNonFixedCell();
...
void CGridCtrl::OnEndInPlaceEdit(NMHDR* pNMHDR, LRESULT* pResult)
{
    ...
    //InvalidateCellRect(CGridCellID(pgvItem->row, pgvItem->col));
...
void CGridCtrl::OnHScroll(UINT nSBCode, UINT /*nPos*/, CScrollBar* /*pScrollBar*/)
{
    ...
    CGridCellID idTopLeft = GetTopleftNonFixedCell();
...
void CGridCtrl::OnDraw(CDC* pDC)
{
    ...
    if (GetVirtualMode())
       SendCacheHintToParent(CGridCellRange(-1,-1,-1,-1));
...
BOOL CGridCtrl::IsValid(const CGridCellID& cell) const
{
...
// of columns/rows)?
BOOL CGridCtrl::IsValid(const CGridCellRange& range) const
{
...
//             GetRuntimeClass()->m_lpszClassName, bAllowDraw? _T("TRUE") : _T("FALSE"));
    ...
    if (bAllowDraw || bResetScrollBars)
        ResetScrollBars();
}

// Forces a redraw of a cell immediately (using a direct DC construction,
// or the supplied dc)
BOOL CGridCtrl::RedrawCell(const CGridCellID& cell, CDC* pDC /* = NULL */)
{
...
CGridCellID CGridCtrl::SetFocusCell(int nRow, int nCol)
{
    return SetFocusCell(CGridCellID(nRow, nCol));
}

CGridCellID CGridCtrl::SetFocusCell(CGridCellID cell)
{
    if (cell == m_idCurrentCell)
        return m_idCurrentCell;

    CGridCellID idPrev = m_idCurrentCell;
...
void CGridCtrl::SetSelectedRange(const CGridCellRange& Range,
                                 ...
                                 BOOL bForceRepaint /* = FALSE */, BOOL bSelectCells/*=TRUE*/)
{
    ...
    CGridCellRange VisCellRange, FixedVisCellRange;
	if (IsWindow(GetSafeHwnd()))
    ...
    if (bSelectCells)
    {
        ...
        for (pos = m_SelectedCellMap.GetStartPosition(); pos != NULL; )
        {
            DWORD key;
            CGridCellID cell;
            m_SelectedCellMap.GetNextAssoc(pos, key, (CGridCellID&)cell);
            ...
            nMinRow >= 0 && nMinCol >= 0 && nMaxRow >= 0 && nMaxCol >= 0)
        {
            for (pos = m_PrevSelectedCellMap.GetStartPosition(); pos != NULL; /* nothing */)
            {
                DWORD key;
                CGridCellID cell;
                m_PrevSelectedCellMap.GetNextAssoc(pos, key, (CGridCellID&)cell);
...
// selects columns
void CGridCtrl::SelectColumns(CGridCellID currentCell, 
...
// selects rows
void CGridCtrl::SelectRows(CGridCellID currentCell, 
                           BOOL bForceRedraw /*=FALSE*/, BOOL bSelectCells /*=TRUE*/)
{
    if (!m_bEnableSelection)
        return;
...
// selects cells
void CGridCtrl::SelectCells(CGridCellID currentCell, 
                            BOOL bForceRedraw /*=FALSE*/, BOOL bSelectCells /*=TRUE*/)
{
    if (!m_bEnableSelection)
        return;
...
void CGridCtrl::OnSelecting(const CGridCellID& currentCell)
{
    if (!m_bEnableSelection)
        return;
...
void CGridCtrl::ClearCells(CGridCellRange Selection)
{
...
BOOL CGridCtrl::PasteTextToGrid(CGridCellID cell, COleDataObject* pDataObject, 
								BOOL bSelectPastedCells /*=TRUE*/)
{
	...
	CGridCellRange PasteRange(cell.row, cell.col,-1,-1);
    int nIndex;
    do
    {
        int nColumn = 0;
        ...
        while (!strLine.IsEmpty())
        {
            ...
            CGridCellID TargetCell(iRowVis, iColVis);
                                 ...
                                 CPoint point)
{
    ...
    CGridCellID cell = GetCellFromPt(point, FALSE);
	bool Valid;
    ...
    {
        OnDragLeave();
        m_LastDragOverCell = CGridCellID(-1,-1);
        return DROPEFFECT_NONE;
    }
	if(m_CurCol==-1)
	{
...
BOOL CGridCtrl::MouseOverRowResizeArea(CPoint& point)
{
    ...
    CGridCellID idCurrentCell = GetCellFromPt(point);
    CPoint start;
...
CGridCellID CGridCtrl::GetCellFromPt(CPoint point, BOOL bAllowFixedCellCheck /*=TRUE*/)
{
    CGridCellID cellID; // return value
...
CGridCellID CGridCtrl::GetTopleftNonFixedCell(BOOL bForceRecalculation /*=FALSE*/)
{
...
CGridCellRange CGridCtrl::GetVisibleNonFixedCellRange(LPRECT pRect /*=NULL*/, 
                                                  BOOL bForceRecalculation /*=FALSE*/)
{
    ...
    CGridCellID idTopLeft = GetTopleftNonFixedCell(bForceRecalculation);
...
CGridCellRange CGridCtrl::GetVisibleFixedCellRange(LPRECT pRect /*=NULL*/, 
                                                  BOOL bForceRecalculation /*=FALSE*/)
{
    int i;
    CRect rect;
    ...
    return CGridCellRange(idTopLeft.row, 0, maxVisibleRow, maxVisibleCol);
}
...
CGridCellRange CGridCtrl::GetUnobstructedNonFixedCellRange(BOOL bForceRecalculation /*=FALSE*/)
{
    CRect rect;
    GetClientRect(rect);
...
CGridCellRange CGridCtrl::GetSelectedCellRange() const
{
...
CGridCellRange CGridCtrl::GetCellRange() const
{
...
BOOL CGridCtrl::GetCellOrigin(const CGridCellID& cell, LPPOINT p)
{
    return GetCellOrigin(cell.row, cell.col, p);
}

// Returns the bounding box of the cell
BOOL CGridCtrl::GetCellRect(const CGridCellID& cell, LPRECT pRect)
{
...
BOOL CGridCtrl::GetTextRect(const CGridCellID& cell, LPRECT pRect)
{
...
BOOL CGridCtrl::GetCellRangeRect(const CGridCellRange& cellRange, LPRECT lpRect)
{
    ...
    if (nHitTest == HTCLIENT)
    {
        ...
        default:
            if (!GetVirtualMode())
            {
                ...
                ScreenToClient(&pt);
                CGridCellID cell = GetCellFromPt(pt);
                if (IsValid(cell))
                {
                    CGridCellBase* pCell = GetCell(cell.row, cell.col);
                    if (pCell)
...
CGridCellID CGridCtrl::GetNextItem(CGridCellID& cell, int nFlags) const
{
    ...
    else if ((nFlags & GVNI_ABOVE) == GVNI_ABOVE) 
    {
        for (int row = cell.row - 1; row >= GetFixedRowCount(); row--) 
        {
            ...
            if ((nFlags & GVNI_DROPHILITED && nState & GVIS_DROPHILITED) || 
                ...
                (nFlags & GVNI_FIXED       && nState & GVIS_FIXED)       ||
                (nFlags & GVNI_MODIFIED    && nState & GVIS_MODIFIED))
                return CGridCellID(row, cell.col);
        }
    }
    else if ((nFlags & GVNI_BELOW) == GVNI_BELOW)
    {
    ...
    else if ((nFlags & GVNI_TOLEFT) == GVNI_TOLEFT)
    {
        for (int col = cell.col - 1; col >= GetFixedColumnCount(); col--) 
        {
            ...
            if ((nFlags & GVNI_DROPHILITED && nState & GVIS_DROPHILITED) || 
                ...
                (nFlags & GVNI_FIXED       && nState & GVIS_FIXED)       ||
                (nFlags & GVNI_MODIFIED    && nState & GVIS_MODIFIED))
                return CGridCellID(cell.row, col);
        }
    }
...
BOOL CGridCtrl::SetItemState(int nRow, int nCol, UINT state)
{
    ...
    if (bSelected && !(state & GVIS_SELECTED))
    {
        ...
        if (m_SelectedCellMap.Lookup(key, (CGridCellID&)cell))
...
BOOL CGridCtrl::SetItemFormatEndEllipsis(int nRow, int nCol, BOOL bUseEndEllipsis) 
{
	UINT format ;
	BOOL ret ;
	format = GetItemFormat( nRow , nCol ) ;
	if( ( format & DT_END_ELLIPSIS ) > 0 ){
		format -= DT_END_ELLIPSIS ;
	}
	ret = SetItemFormat( nRow , nCol , format ) ;
                               ...
                               BOOL bResetScroll /*=TRUE*/)
{
    ...
    if (GetVirtualMode())
        SendCacheHintToParent(CGridCellRange(nStartRow, nCol, nEndRow, nCol));
...
BOOL CGridCtrl::AutoSizeRow(int nRow, BOOL bResetScroll /*=TRUE*/)
{
    ...
    if (GetVirtualMode())
        SendCacheHintToParent(CGridCellRange(nRow, 0, nRow, nNumColumns));
...
void CGridCtrl::AutoSize(UINT nAutoSizeStyle /*=GVS_DEFAULT*/)
{
    ...
    if (GetVirtualMode())
        SendCacheHintToParent(CGridCellRange(nStartRow, 0, nEndRow, nNumColumns));
...
void CGridCtrl::ExpandColumnsToFit(BOOL bExpandFixed /*=TRUE*/)
{
    ...
    int nColumnAdjustment = rect.Width() / nNumColumnsAffected;
    for (col = nFirstColumn; col < GetColumnCount(); col++)
    {
        if (m_arColWidths[col] > 0)
...
void CGridCtrl::SetImageListRow(int nRow, CImageList *pImages)
{
	if ( nRow < 0 )  
	{
		ASSERT( FALSE ) ; return ;
	}
	if ( this->GetRowCount() <= nRow )  
	{
		ASSERT( FALSE ) ; return ;
	}
	...
	{
		GetCell( nRow , col )->SetImageList( pImages ) ;
	}
}
void CGridCtrl::SetImageListCol(int nCol, CImageList *pImages)
{
	if ( nCol < 0 ) 
	{
		ASSERT( FALSE ) ; return ;
	...
	{
		GetCell( row , nCol )->SetImageList( pImages ) ;
	}
}
void CGridCtrl::SetImageListCell(int nRow, int nCol , CImageList *pImages)
{
if ( nRow < 0 )  
	{
		ASSERT( FALSE ) ; return ;
	...
	// 画像をセット
	GetCell( nRow , nCol )->SetImageList( pImages ) ;
...
//KANG 22.06.17 Add End 
        ...
        CGridCellID idTopLeft = GetTopleftNonFixedCell(FALSE);
        CGridCellID idNewTopLeft = GetTopleftNonFixedCell(TRUE);
        if (idNewTopLeft != idTopLeft)
        {
...
///*
    else if (m_nEnsureVisibleType == 1) {
...
BOOL CGridCtrl::IsCellEditable(CGridCellID &cell) const
{
...
BOOL CGridCtrl::IsCellSelected(CGridCellID &cell) const
{
...
BOOL CGridCtrl::IsCellSelected(int nRow, int nCol) const
{
    ...
    {   
        ...
        return (m_SelectedCellMap.Lookup(key, (CGridCellID&)cell));       
    }
    else
        return IsSelectable() && ((GetItemState(nRow, nCol) & GVIS_SELECTED) == GVIS_SELECTED);
}

BOOL CGridCtrl::IsCellVisible(CGridCellID cell) 
{
...
BOOL CGridCtrl::InvalidateCellRect(const CGridCellID& cell)
{
...
BOOL CGridCtrl::InvalidateCellRect(const CGridCellRange& cellRange)
{
...
//						TRACE0("Showing TitleTip\n");
						...
						m_TitleTip.Show(TextRect, pCell->GetTipText(), xoffset, CellRect,
...
void CGridCtrl::OnLButtonDblClk(UINT nFlags, CPoint point)
{
    ...
    CGridCellID cell = GetCellFromPt(point);
    if( !IsValid( cell) )
    {
        //ASSERT(FALSE);
        return;
    ...
    else if (m_MouseMode == MOUSE_NOTHING)
    {
        ...
        if (IsValid(cell))
        {
			...
			if (pCell != NULL && pCell->IsReadOnly() > FALSE ){
...
void CGridCtrl::OnRButtonUp(UINT nFlags, CPoint point)
{
    ...
    CGridCellID FocusCell;
    FocusCell = GetCellFromPt(point);

    EndEditing();        // Auto-destroy any GridInPlaceEdit's
...
void CGridCtrl::OnFixedRowClick(CGridCellID& cell)
{
    if (!IsValid(cell))
        return;
...
void CGridCtrl::OnFixedColumnClick(CGridCellID& cell)
{
    if (!IsValid(cell))
        return;
...
CGridCellID CGridCtrl::GetMergeCellID(CGridCellID cell)
{
...
CGridCellBase* CGridCtrl::GetCell(CGridCellID cell)
{
...
{
	CGridCellRange range=GetSelectedCellRange();
...
void CGridCtrl::MergeCells(int nStartRow, int nStartCol, int nEndRow, int nEndCol)
{
	for(int row=nStartRow;row<=nEndRow;row++)
	{
		for(int col=nStartCol;col<=nEndCol;col++)
		{
			...
			{
				CGridCellRange range(nStartRow,  nStartCol,  nEndRow,  nEndCol);
				pCell->SetMergeRange(range);
			}
			else
			{
				CGridCellID cell(nStartRow,nStartCol);
...
int  CGridCtrl::GetMergeCellWidth(CGridCellID cell)
{
	CGridCellID mergecell =GetMergeCellID(cell);
...
int  CGridCtrl::GetMergeCellHeight(CGridCellID cell)
{
	CGridCellID mergecell =GetMergeCellID(cell);
    CGridCellBase *pCell = (CGridCellBase*) GetCell(mergecell);
	if(!pCell->IsMerged())
...
void CGridCtrl::SetCellAlignment( int row, int column, DWORD pos )
{
	if( pos > flexAlignGeneral ){
		return ;
	}

	
	UINT nFormat = this->GetItemFormat( row, column ) ;
	nFormat &= 0xffffffe0 ;					// see "#define DT_TOP 0x00000000" @WinUser.h  et. al

	switch( pos ){
		case flexAlignLeftTop :
			nFormat |= ( DT_LEFT | DT_TOP ) ;
			break ;
		...
		case flexAlignRightCenter : 
			nFormat |= ( DT_RIGHT | DT_VCENTER ) ;
			break ;

		case flexAlignRightBottom :
			nFormat |= ( DT_RIGHT | DT_BOTTOM ) ;
			break ;
	
		case flexAlignGeneral :
		{
			CString str = this->GetItemText( row, column ) ;
			int value ;

#if _MSC_VER > 1200
			int res = ::_stscanf_s( str, _T("%d"), &value ) ;
#else
			int res = ::sscanf_s( str, "%d", &value ) ;
#endif
			if( res > 0 ){
				nFormat |= ( DT_RIGHT | DT_VCENTER ) ;
			}
	...
	this->SetItemFormat( row, column, nFormat ) ;
}

void CGridCtrl::SetRowAlignment( int row, DWORD pos )
{
	for( int iCol = 0 ; iCol < this->GetCols() ; iCol++ ){
		this->SetCellAlignment( row, iCol, pos ) ;
	}
}

void CGridCtrl::SetColAlignment( int column, DWORD pos )
{
	for( int iRow = 0 ; iRow < this->GetRows() ; iRow++ ){
		this->SetCellAlignment( iRow, column, pos ) ;
	}
}


void CGridCtrl::SetFixedAlignment( int row, int column, DWORD pos )
{
	this->SetCellAlignment( row, column, pos ) ;
}


long CGridCtrl::GetRowSel( void )
{
	return( (long)( this->GetSelectedCellRange().GetMaxRow() ) ) ;
}

long CGridCtrl::GetColSel( void )
{
	return( (long)( this->GetSelectedCellRange().GetMaxCol() ) ) ;
}


long CGridCtrl::GetCellLeft( void )
{
	RECT rc ;
	CGridCellID cid = this->GetFocusCell() ;
	BOOL res = this->GetCellRect( cid.row, cid.col, &rc ) ;

	long pos_left = ( res == TRUE ) ? rc.left : -1 ;

	return( pos_left ) ;
}

long CGridCtrl::GetCellTop( void )
{
	RECT rc ;
	CGridCellID cid = this->GetFocusCell() ;
	BOOL res = this->GetCellRect( cid.row, cid.col, &rc ) ;

	long pos_top = ( res == TRUE ) ? rc.top : -1 ;

	return( pos_top ) ;
}

long CGridCtrl::GetCellWidth( void )
{
	RECT rc ;
	CGridCellID cid = this->GetFocusCell() ;
	BOOL res = this->GetCellRect( cid.row, cid.col, &rc ) ;

	long width = ( res == TRUE ) ? rc.right - rc.left : -1 ;

	return( width ) ;
}

long CGridCtrl::GetCellHeight( void )
{
	RECT rc ;
	CGridCellID cid = this->GetFocusCell() ;
	BOOL res = this->GetCellRect( cid.row, cid.col, &rc ) ;

	long height = ( res == TRUE ) ? rc.bottom - rc.top : -1 ;

	return( height ) ;
}


void CGridCtrl::SetScrollTrack( BOOL bNewValue )
{

}


void CGridCtrl::SetCellBackColor( int row, int column, COLORREF clr )
{
    if (row < 1) return;
    if (row > m_nRows) {
        return;
    }
	CGridCellBase *pGcb = this->GetCell( row, column ) ;
	pGcb->SetBackClr( clr ) ;
}

COLORREF CGridCtrl::GetCellBackColor( int row, int column )
{
	COLORREF clr ;	
    if (row < 1) return( RGB(100, 100, 100 ) ) ;
...
void CGridCtrl::SetCellTextColor( int row, int column, COLORREF clr )
{
    if (row < 1) return;
    if (row > m_nRows) {
        return;
...
void CGridCtrl::SetCellBackColorAll( COLORREF clr )
{
	for ( int row = GetFixedRowCount() ; row < GetRowCount() ; row++ )
	{
		for ( int col = GetFixedColumnCount() ; col < GetColumnCount() ; col++ )
		{
			SetCellBackColor( row , col , clr ) ;
		}
	}
}

void CGridCtrl::ResetEditableInit()
{
	SetEditable( FALSE ) ;
	...
	for ( int row = 0 ; row < GetRowCount() ; row++ )
	{
		for ( int col = 0 ; col < GetColumnCount() ; col++ )
		{
			DWORD dwState = GetCell( row , col )->GetState() ;
			dwState |= GVIS_READONLY ;
			GetCell( row , col )->SetState( dwState ) ;
		}
	}
}

void CGridCtrl::SetEditableCol( int nEditableCol )
{
	SetEditable( TRUE ) ;
//	if ( m_bSetEditableInit == FALSE )
//	{
//		for ( int row = 0 ; row < GetRowCount() ; row++ )
//		{
//			for ( int col = 0 ; col < GetColumnCount() ; col++ )
...
//			}
//		}
//		m_bSetEditableInit = TRUE ;
//	}
	if ( nEditableCol < 0 )  
	{
		ASSERT( FALSE ) ; return ;
	}
	if ( GetColumnCount() <= nEditableCol )  
	{
		ASSERT( FALSE ) ; return ;
	}
	...
	for ( int row = GetFixedRowCount() ; row < GetRowCount() ; row++ )
	{
		DWORD dwState = GetCell( row , nEditableCol )->GetState() ;

		dwState |= GVIS_READONLY ;
		dwState ^= GVIS_READONLY ;

		GetCell( row , nEditableCol )->SetState( dwState ) ;
	}
}

void CGridCtrl::SetEditableRow( int nEditableRow )
{
	SetEditable( TRUE ) ;
//	if ( m_bSetEditableInit == FALSE )
//	{
...
//	}
	...
	if ( this->GetRowCount() <= nEditableRow )  
	{
		ASSERT( FALSE ) ; return ;
	}
	...
	for ( int col = GetFixedColumnCount() ; col < GetColumnCount() ; col++ )
	{
		DWORD dwState = GetCell( nEditableRow , col )->GetState() ;

		dwState |= GVIS_READONLY ;
		dwState ^= GVIS_READONLY ;

		GetCell( nEditableRow , col )->SetState( dwState ) ;
	}
}

void CGridCtrl::SetEditableCell( int nEditableRow , int nEditableCol )
{
	SetEditable( TRUE ) ;
...
//	}
	...
	if ( this->GetColumnCount() <= nEditableCol )  
	{
		ASSERT( FALSE ) ; return ;
	}

	{
		DWORD dwState = GetCell( nEditableRow , nEditableCol )->GetState() ;

		dwState |= GVIS_READONLY ;
		dwState ^= GVIS_READONLY ;

		GetCell( nEditableRow , nEditableCol )->SetState( dwState ) ;
	}
}

void CGridCtrl::SetReadOnlyCell( int nRow , int nCol )
{
	DWORD dwState = GetCell( nRow , nCol )->GetState() ;

	dwState |= GVIS_READONLY ;

	GetCell( nRow , nCol )->SetState( dwState ) ;
}

void CGridCtrl::SetComboBox( int row, int column, CStringArray& arList , DWORD dwStyle /*= CBS_DROPDOWNLIST*/)
{
    if (!SetCellType(row,column, RUNTIME_CLASS(CGridCellCombo)))
	{
		ASSERT( FALSE ) ;
        return;
	}

    CGridCellCombo *pCell = (CGridCellCombo*) GetCell(row,column);

    pCell->SetOptions(arList);
    pCell->SetStyle(dwStyle); //CBS_DROPDOWN, CBS_DROPDOWNLIST, CBS_SIMPLE
	pCell->SetCurSel( 0 ) ;

}

void CGridCtrl::SetComboBoxCol( int nComboBoxCol, CStringArray& arList , DWORD dwStyle /*= CBS_DROPDOWNLIST*/)
{
	if ( nComboBoxCol < 0 ) 
	{
		ASSERT( FALSE ) ; return ;
	}
	if ( GetColumnCount() <= nComboBoxCol ) 
	{
		ASSERT( FALSE ) ; return ;
	}

	SetEditableCol( nComboBoxCol ) ;

	for ( int row = GetFixedRowCount() ; row < GetRowCount() ; row++ )
	{
		SetComboBox( row , nComboBoxCol , arList , dwStyle ) ;
	}
}

void CGridCtrl::SetComboBoxRow( int nComboBoxRow, CStringArray& arList , DWORD dwStyle /*= CBS_DROPDOWNLIST*/)
{
	if ( nComboBoxRow < 0 )  
	{
		ASSERT( FALSE ) ; return ;
	}
	if ( this->GetRowCount() <= nComboBoxRow )  
	{
		ASSERT( FALSE ) ; return ;
	}
	
	SetEditableRow( nComboBoxRow ) ;

	for ( int col = GetFixedColumnCount() ; col < GetColumnCount() ; col++ )
	{
		SetComboBox( nComboBoxRow , col , arList , dwStyle ) ;
	}
}


void CGridCtrl::SetComboBoxCell( int nComboBoxRow , int nComboBoxCol , CStringArray& arList , DWORD dwStyle /*= CBS_DROPDOWNLIST*/)
{
	if ( nComboBoxRow < 0 )  
	{
		ASSERT( FALSE ) ; return ;
	...
	if ( this->GetColumnCount() <= nComboBoxCol )  
	{
		ASSERT( FALSE ) ; return ;
	}

	SetEditableCell( nComboBoxRow , nComboBoxCol ) ;
	SetComboBox( nComboBoxRow , nComboBoxCol, arList , dwStyle ) ;
}

void CGridCtrl::SetCheckBox( int row, int column )
{
    if (!SetCellType(row,column, RUNTIME_CLASS(CGridCellCheck)))
	{
		ASSERT( FALSE ) ;
        return;
	}
...
//void CGridCtrl::SetCheckBoxCol( int nCheckBoxCol )
void CGridCtrl::SetCheckBoxCol( int nCheckBoxCol , BOOL bCallBack )
{
	if ( nCheckBoxCol < 0 ) 
	{
		ASSERT( FALSE ) ; return ;
	}
	if ( GetColumnCount() <= nCheckBoxCol ) 
	{
		ASSERT( FALSE ) ; return ;
	}

	if( bCallBack == TRUE ){
		SetEditableCol( nCheckBoxCol ) ;
	}else{
		for ( int row = GetFixedRowCount() ; row < GetRowCount() ; row++ )
		{
			SetReadOnlyCell( row , nCheckBoxCol ) ;
		}
	}

	for ( int row = GetFixedRowCount() ; row < GetRowCount() ; row++ )
	{
		SetCheckBox( row , nCheckBoxCol ) ;
	}
}

//void CGridCtrl::SetCheckBoxRow( int nCheckBoxRow )
void CGridCtrl::SetCheckBoxRow( int nCheckBoxRow , BOOL bCallBack  )
{
	if ( nCheckBoxRow < 0 )  
	{
		ASSERT( FALSE ) ; return ;
	}
	if ( this->GetRowCount() <= nCheckBoxRow )  
	{
		ASSERT( FALSE ) ; return ;
	}

	if( bCallBack == TRUE ){
		SetEditableRow( nCheckBoxRow ) ;
	}else{
		for ( int col = GetFixedColumnCount() ; col < GetColumnCount() ; col++ )
		{
			SetReadOnlyCell( nCheckBoxRow , col ) ;
		}
	}

	for ( int col = GetFixedColumnCount() ; col < GetColumnCount() ; col++ )
	{
		SetCheckBox( nCheckBoxRow , col ) ;
	}
}


//void CGridCtrl::SetCheckBoxCell( int nCheckBoxRow , int nCheckBoxCol )
void CGridCtrl::SetCheckBoxCell( int nCheckBoxRow , int nCheckBoxCol , BOOL bCallBack )
{
	if ( nCheckBoxRow < 0 )  
	{
		ASSERT( FALSE ) ; return ;
	...
	if ( this->GetColumnCount() <= nCheckBoxCol )  
	{
		ASSERT( FALSE ) ; return ;
	}

	if( bCallBack == TRUE ){
		SetEditableCell( nCheckBoxRow , nCheckBoxCol ) ;
	}else{
		SetReadOnlyCell( nCheckBoxRow , nCheckBoxCol ) ;
	}

	SetCheckBox( nCheckBoxRow , nCheckBoxCol ) ;
}

void CGridCtrl::SetSpinCtrl( int row, int column )
{
    if (!SetCellType(row,column, RUNTIME_CLASS(CGridCellSpinCtrl)))
	{
		ASSERT( FALSE ) ;
        return;
	}
...
void CGridCtrl::SetCellClick( int row, int column , int nClickMode )
{
    if (!SetCellType(row,column, RUNTIME_CLASS(CGridCellClick)))
	{
		ASSERT( FALSE ) ;
        return;
	}

    CGridCellClick *pCell = (CGridCellClick*) GetCell(row,column);
	BOOL bDoubleClick = ( nClickMode == enDoubleClick ? TRUE : FALSE ) ;
	pCell->SetDClick( bDoubleClick );
}

void CGridCtrl::SetSpinCtrlCol( int nSpinCtrlCol )
{
	if ( nSpinCtrlCol < 0 ) 
	{
		ASSERT( FALSE ) ; return ;
	}
	if ( GetColumnCount() <= nSpinCtrlCol ) 
	{
		ASSERT( FALSE ) ; return ;
	}

	SetEditableCol( nSpinCtrlCol ) ;
	for ( int row = GetFixedRowCount() ; row < GetRowCount() ; row++ )
	{
		SetSpinCtrl( row , nSpinCtrlCol ) ;
	}
}

void CGridCtrl::SetSpinCtrlRow( int nSpinCtrlRow )
{
	if ( nSpinCtrlRow < 0 )  
	{
		ASSERT( FALSE ) ; return ;
	}
	if ( this->GetRowCount() <= nSpinCtrlRow )  
	{
		ASSERT( FALSE ) ; return ;
	}
	
	SetEditableRow( nSpinCtrlRow ) ;
	for ( int col = GetFixedColumnCount() ; col < GetColumnCount() ; col++ )
	{
		SetSpinCtrl( nSpinCtrlRow , col ) ;
	}
}

void CGridCtrl::SetSpinCtrlCell( int nSpinCtrlRow , int nSpinCtrlCol )
{
	if ( nSpinCtrlRow < 0 )  
	{
		ASSERT( FALSE ) ; return ;
	...
	if ( this->GetColumnCount() <= nSpinCtrlCol )  
	{
		ASSERT( FALSE ) ; return ;
	}

	SetEditableCell( nSpinCtrlRow , nSpinCtrlCol ) ;
	SetSpinCtrl( nSpinCtrlRow , nSpinCtrlCol ) ;
}

void CGridCtrl::SetClickCol( int nCellClickCol , int nClickMode )
{
	if ( nCellClickCol < 0 ) 
	{
		ASSERT( FALSE ) ; return ;
	}
	if ( GetColumnCount() <= nCellClickCol ) 
	{
		ASSERT( FALSE ) ; return ;
	}

	SetEditableCol( nCellClickCol ) ;
	for ( int row = GetFixedRowCount() ; row < GetRowCount() ; row++ )
	{
		SetCellClick( row , nCellClickCol , nClickMode ) ;
	}
}

void CGridCtrl::SetClickRow( int nCellClickRow , int nClickMode )
{
	if ( nCellClickRow < 0 )  
	{
		ASSERT( FALSE ) ; return ;
	}
	if ( this->GetRowCount() <= nCellClickRow )  
	{
		ASSERT( FALSE ) ; return ;
	}
	
	SetEditableRow( nCellClickRow ) ;
	for ( int col = GetFixedColumnCount() ; col < GetColumnCount() ; col++ )
	{
		SetCellClick( nCellClickRow , col , nClickMode ) ;
	}
}

void CGridCtrl::SetClickCell( int nCellClickRow , int nCellClickCol , int nClickMode )
{
	if ( nCellClickRow < 0 )  
	{
		ASSERT( FALSE ) ; return ;
	...
	if ( this->GetColumnCount() <= nCellClickCol )  
	{
		ASSERT( FALSE ) ; return ;
	}

	SetEditableCell( nCellClickRow , nCellClickCol ) ;
	SetCellClick( nCellClickRow , nCellClickCol , nClickMode ) ;
}

void CGridCtrl::NotifyChangeCell( int nRow , int nCol )
{
...
int CGridCtrl::GetTopRow( void )
{
	return( 0 ) ;
}

void CGridCtrl::SetTopRow( int row )
{

}

//KANG 22.05.31 Add Start
void CGridCtrl::SetEnsureVisibleType(int nType)
{
