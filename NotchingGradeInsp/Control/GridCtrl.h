...
#define GRIDCTRL_CLASSNAME    _T("GridCtrl")  // Window class name
...
// Construction
public:
    CGridCtrl(int nRows = 2, int nCols = 2, int nFixedRows = 1, int nFixedCols = 1);
...
public:
    ...
    int  GetRows() const						{ return GetRowCount() ; }
    int  GetColumnCount() const                 { return m_nCols; }
    int  GetCols() const						{ return GetColumnCount() ; }
    int  GetFixedRowCount() const               { return m_nFixedRows; }
    int  GetFixedColumnCount() const            { return m_nFixedCols; }
    BOOL SetRowCount(int nRows = 10);
    BOOL SetRows( int nRows ) { return SetRowCount( nRows ) ; }
    BOOL SetColumnCount(int nCols = 10);
    BOOL SetCols( int nCols ) { return SetColumnCount( nCols ) ; }
    BOOL SetFixedRows( int nFixedRows ) { return SetFixedRowCount( nFixedRows ) ; }
    BOOL SetFixedRowCount(int nFixedRows = 1);
    BOOL SetFixedCols( int nFixedCols ) { return SetFixedColumnCount( nFixedCols ) ; }
	...
	{ 
		int nRet =  GetColumnWidth( nCol ) ;
		return ( ( int)( nRet / m_xx + 0.5 ) ) ;
	}
    BOOL SetColumnWidth(int col, int width);
    BOOL SetColWidth( int col, int width )
	{
		return ( SetColumnWidth( col, ( int)( width * m_xx + 0.5 ) ) ) ;
	}

    BOOL GetCellOrigin(int nRow, int nCol, LPPOINT p);
    BOOL GetCellOrigin(const CGridCellID& cell, LPPOINT p);
    BOOL GetCellRect(int nRow, int nCol, LPRECT pRect);
    BOOL GetCellRect(const CGridCellID& cell, LPRECT pRect);

    BOOL GetTextRect(const CGridCellID& cell, LPRECT pRect);
    BOOL GetTextRect(int nRow, int nCol, LPRECT pRect);

    CGridCellID GetCellFromPt(CPoint point, BOOL bAllowFixedCellCheck = TRUE);
    ...
    CGridCellID SetFocusCell(CGridCellID cell);
    CGridCellID SetFocusCell(int nRow, int nCol);
    CGridCellID GetFocusCell() const                  { return m_idCurrentCell;           }
	int GetRow() const { return ( GetFocusCell().row ) ; }
	int GetCol() const { return ( GetFocusCell().col ) ; }
	int GetMouseRow() ;
	int GetMouseCol() ;
	...
	void SetImageListRow(int nRow, CImageList *pImages) ;
	void SetImageListCol(int nCol, CImageList *pImages) ;
	void SetImageListCell(int nRow, int nCol, CImageList *pImages) ;
...
public:
	...
	BOOL   SetTextMatrix( int nRow, int nCol, LPCTSTR str ) { return ( SetItemText( nRow, nCol, str ) ) ; }
    ...
    CString GetTextMatrix( int nRow, int nCol ) const { return ( GetItemText( nRow, nCol ) ) ; }
	...
	BOOL   SetItemFormatEndEllipsis(int nRow, int nCol, BOOL bUseEndEllipsis);
...
public:
	...
	void ClearCells(CGridCellRange Selection);
    ...
    void EnsureVisible(CGridCellID &cell)       { EnsureVisible(cell.row, cell.col); }
    void EnsureVisible(int nRow, int nCol);
    BOOL IsCellVisible(int nRow, int nCol);
    BOOL IsCellVisible(CGridCellID cell);
    BOOL IsCellEditable(int nRow, int nCol) const;
    BOOL IsCellEditable(CGridCellID &cell) const;
    BOOL IsCellSelected(int nRow, int nCol) const;
    BOOL IsCellSelected(CGridCellID &cell) const;
    ...
    BOOL RedrawCell(const CGridCellID& cell, CDC* pDC = NULL);
...
///////////////////////////////////////////////////////////////////////////////////
 public:
    ...
    void SetSelectedRange(const CGridCellRange& Range, BOOL bForceRepaint = FALSE, BOOL bSelectCells = TRUE);
    void SetSelectedRange(int nMinRow, int nMinCol, int nMaxRow, int nMaxCol,
                          BOOL bForceRepaint = FALSE, BOOL bSelectCells = TRUE);
    BOOL IsValid(int nRow, int nCol) const;
    BOOL IsValid(const CGridCellID& cell) const;
...
///////////////////////////////////////////////////////////////////////////////////
    ...
    virtual BOOL PasteTextToGrid(CGridCellID cell, COleDataObject* pDataObject, BOOL bSelectPastedCells=TRUE);
#endif
...
///////////////////////////////////////////////////////////////////////////////////
public:
    CGridCellID GetNextItem(CGridCellID& cell, int nFlags) const;
...
// Implementation
public:
	CGridCellBase* GetCell(CGridCellID cell);
	CGridCellID GetMergeCellID(CGridCellID cell);
	...
	int  GetMergeCellWidth(CGridCellID cell);
	int  GetMergeCellHeight(CGridCellID cell);
	BOOL GetCellOriginNoMerge(int nRow, int nCol, LPPOINT p);
	BOOL GetCellOriginNoMerge(const CGridCellID& cell, LPPOINT p);
...
protected:
    ...
    LRESULT SendCacheHintToParent(const CGridCellRange& range) const;

    BOOL InvalidateCellRect(const int row, const int col);
    BOOL InvalidateCellRect(const CGridCellID& cell);
    BOOL InvalidateCellRect(const CGridCellRange& cellRange);
    void EraseBkgnd(CDC* pDC);

    BOOL GetCellRangeRect(const CGridCellRange& cellRange, LPRECT lpRect);
    ...
    CGridCellID GetTopleftNonFixedCell(BOOL bForceRecalculation = FALSE);
    CGridCellRange GetUnobstructedNonFixedCellRange(BOOL bForceRecalculation = FALSE);
    CGridCellRange GetVisibleNonFixedCellRange(LPRECT pRect = NULL, BOOL bForceRecalculation = FALSE);
    CGridCellRange GetVisibleFixedCellRange(LPRECT pRect = NULL, BOOL bForceRecalculation = FALSE);
...
protected:
    ...
    // Mouse Clicks
    virtual void  OnFixedColumnClick(CGridCellID& cell);
    virtual void  OnFixedRowClick(CGridCellID& cell);
...
protected:
    ...
    CGridCellID     m_LeftClickDownCell, m_SelectionStartCell;
    CGridCellID     m_idCurrentCell, m_idTopLeftCell;
    INT_PTR     m_nTimerID;
    int         m_nTimerInterval;
    int         m_nResizeCaptureRange;
    BOOL        m_bAllowRowResize, m_bAllowColumnResize;
    int         m_nRowsPerWheelNotch;
    CMap<DWORD,DWORD, CGridCellID, CGridCellID&> m_SelectedCellMap, m_PrevSelectedCellMap;

#ifndef GRIDCONTROL_NO_TITLETIPS
    CGridTitleTip   m_TitleTip;             // Title tips for cells
#endif

    // Drag and drop
    CGridCellID     m_LastDragOverCell;
#ifndef GRIDCONTROL_NO_DRAGDROP
...
protected:
    void SelectAllCells();
    void SelectColumns(CGridCellID currentCell, BOOL bForceRedraw=FALSE, BOOL bSelectCells=TRUE);
    void SelectRows(CGridCellID currentCell, BOOL bForceRedraw=FALSE, BOOL bSelectCells=TRUE);
    void SelectCells(CGridCellID currentCell, BOOL bForceRedraw=FALSE, BOOL bSelectCells=TRUE);
    void OnSelecting(const CGridCellID& currentCell);
...
//KANG 22.06.17 Add Start
	int m_nEnsureVisibleType;
//KANG 22.06.17 Add End

public :
	void	SetCellAlignment( int row, int column, DWORD pos ) ;
	void	SetRowAlignment( int row, DWORD pos ) ;
	void	SetColAlignment( int column, DWORD pos ) ;
	void	SetFixedAlignment( int row, int column, DWORD pos ) ;
	long	GetRowSel( void ) ;
	long	GetColSel( void ) ;
	...
	void	SetCellBackColor( int row, int column, COLORREF clr ) ;
	COLORREF GetCellBackColor( int row, int column ) ;
	void	SetCellTextColor( int row, int column, COLORREF clr ) ;
	void	SetCellBackColorAll( COLORREF clr ) ;
	void	ResetEditableInit() ;
	void	SetEditableCol( int nEditableCol ) ;
	void	SetEditableRow( int nEditableRow ) ;
	void	SetEditableCell( int nEditableRow , int nEditableCol ) ;
	void	SetReadOnlyCell( int nRow , int nCol ) ;
	void	SetComboBoxCol( int nComboBoxCol, CStringArray& arList , DWORD dwStyle = CBS_DROPDOWNLIST) ;
	void	SetComboBoxRow( int nComboBoxRow, CStringArray& arList , DWORD dwStyle = CBS_DROPDOWNLIST) ;
	void	SetComboBoxCell( int nComboBoxRow, int nComboBoxCol, CStringArray& arList , DWORD dwStyle = CBS_DROPDOWNLIST) ;
//	void    SetCheckBoxCol( int nCheckBoxCol ) ;
//	void    SetCheckBoxRow( int nCheckBoxRow ) ;
//	void    SetCheckBoxCell( int nCheckBoxRow , int nCheckBoxCol ) ;
	void    SetCheckBoxCol( int nCheckBoxCol , BOOL bCallBack ) ;
	void    SetCheckBoxRow( int nCheckBoxRow , BOOL bCallBack ) ;
	void    SetCheckBoxCell( int nCheckBoxRow , int nCheckBoxCol ,  BOOL bCallBack ) ;
	void	SetSpinCtrlCol( int nSpinCtrlCol ) ;
	void	SetSpinCtrlRow( int nSpinCtrlRow ) ;
	void	SetSpinCtrlCell( int nSpinCtrlRow, int nSpinCtrlCol ) ;
	void	NotifyChangeCell( int nRow , int nCol ) ;
	enum{
		enSingleClick ,		// 左クリック
		enDoubleClick ,		// 左ダブルクリック
	} ;
	void	SetClickCol( int nCellClickCol , int nClickMode ) ;
	void	SetClickRow( int nCellClickRow , int nClickMode ) ;
	void	SetClickCell( int nCellClickRow, int nCellClickCol , int nClickMode ) ;

	int     GetTopRow( void ) ;
	void    SetTopRow( int row ) ;
//KANG 22.06.17 Add Start
	void    SetEnsureVisibleType(int nType);
//KANG 22.06.17 Add End

private:
//	BOOL	m_bSetEditableInit ;
	void	SetComboBox( int row, int column, CStringArray& arList , DWORD dwStyle = CBS_DROPDOWNLIST ) ;

	
//	void SetCheckBox( int row, int column ) ;
	void SetCheckBox( int row, int column ) ;
...
inline CGridCellBase* CGridCtrl::GetCell(int nRow, int nCol) const
{
    ...
    if (GetVirtualMode())
    {
        ...
        if (GetFocusCell() == CGridCellID(nRow, nCol)) gvdi.item.nState |= GVIS_FOCUSED;
		if(!m_InDestructor)
		{
